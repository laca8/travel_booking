...
import { editHorse } from "../../redux/slicers/horseSlicer";
...
const EditHorse = ({ id, isOpen, setIsOpen }) => {
  ...
  const handleUpdate = () => {
    ...
    dispatch(editHorse(formData));
    if (success) {
      setNotify(toast.success("تم تحديث بيانات الخيل"));
    }
  };
  useEffect(() => {
  ...
  useEffect(() => {
    if (horse) {
      setName(horse?.data?.name);
      setAge(horse?.data?.age);
      setSex(horse?.data?.sex);
      setType(horse?.data?.type);
      setColor(horse?.data?.color);
      setStable(horse?.data?.stable);
      setOwner(horse?.data?.horse_owner);
      setShip(horse?.data?.ship_num);
      setBuying(horse?.data?.buying);
      setChamp(horse?.data?.champions);
      setPulse(horse?.data?.pulse);
      setHeat(horse?.data?.heat);
      setBlood(horse?.data?.blood);
      setEye(horse?.data?.eye);
      setFile(horse?.data?.image);
    }
  }, [horse]);
    ...
    <div>
      <div className="p-4">
        <button
          ...
          className="mt-4 px-6 py-2.5  text-sm bg-green-500 text-white rounded hover:bg-green-600 transition-all">
        ...
        <Modal
          ...
          title="بيانات الخيل">
          <div className="w-full relative grid grid-cols-3 gap-2 max-md:grid-cols-2 border-2 border-[var(--dark-color)] mb-2">
